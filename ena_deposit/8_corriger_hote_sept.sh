#!/bin/bash
# Correction de l'IDENTITE D'HOTE du depot ENA PRJEB124417 (action MODIFY), classification
# genotypique a 25 chromosomes (septembre 2026). Derive de 4_corriger_metadonnees.sh : meme
# transport, meme analyse du recu, meme garde-fou sur les accessions.
#
#   python3 ../scripts/28-ena_hote_sept.py        # construit ena_update_sept{,_test}/ (deja fait)
#   bash 8_corriger_hote_sept.sh test             # serveur de test : valide les VALEURS
#   bash 8_corriger_hote_sept.sh prod             # POUR DE VRAI
#   bash 9_verifier_hote_sept.sh                  # verification exhaustive, quelques heures apres
#
# Soumet 568 echantillons (TITLE, host scientific name, host common name). Ne touche ni aux
# 41 controles, ni aux 159 echantillons deja corrects, ni aux FASTQ, EXPERIMENT ou RUN.
# Le mode test utilise des alias suffixes figes a la construction : pour retester, relancer
# d'abord scripts/28-ena_hote_sept.py (nouveau suffixe).
set -uo pipefail
MODE="${1:-}"
...
if [ "$MODE" = "test" ]; then
  ...
  for f in submission.xml sample.xml; do
    [ -f "$D/ena_update_sept_test/$f" ] || { echo "ERREUR : ena_update_sept_test/$f introuvable (lancer scripts/28-ena_hote_sept.py)"; exit 1; }
  done

  if ! envoyer ena_update_sept_test/submission.xml ena_update_sept_test/sample.xml "hote_sept_test"; then
    unset P; exit 1
  fi
  analyser "$RECU" "-"; RC=$?
  unset P
  ...
  if [ $RC -eq 0 ]; then
    ...
    echo "titres et noms d'hote corriges."
    echo
...
# =====================================================================
XML=$D/ena_update_sept
for f in submission.xml sample.xml; do
  [ -f "$XML/$f" ] || { echo "ERREUR : $XML/$f introuvable (lancer scripts/28-ena_hote_sept.py)"; exit 1; }
done
...
echo "=== Correction de l'identite d'hote, 25 chromosomes (PRODUCTION) ==="
...
if ! envoyer "$XML/submission.xml" "$XML/sample.xml" "hote_sept_prod"; then
  unset P; exit 3
fi
analyser "$RECU" "$REF"; RC=$?
unset P
echo
[ $RC -eq 0 ] && { echo "CORRECTION APPLIQUEE. Verifiez dans quelques heures :"; echo "  bash 9_verifier_hote_sept.sh"; }
exit $RC
