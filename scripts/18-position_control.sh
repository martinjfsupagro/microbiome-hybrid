...
"$RSCRIPT" - <<'RS' > results/position_effect/position_control.txt
suppressMessages({library(vegan); library(permute)})
set.seed(20260830)
NPERM <- 999
NCPU  <- as.integer(Sys.getenv("SLURM_CPUS_PER_TASK", "8"))
TSV   <- "results/position_effect/position_control.tsv"
...
for (met in c("bray","jaccard")) {
  ...
  for (sub in c("mono_taxon","multi_taxon")) {
    sites <- if (sub=="mono_taxon") MONO else MULTI
    cat(sprintf("\n=== sous-ensemble %s : sites %s ===\n", sub, paste(sites, collapse=", ")))
    for (run in c("durance1","durance2","durance3")) {
      for (tis in c("caudale","branchie","midgut","hindgut")) {
        ...
        sel <- !is.na(md$tissue) & md$tissue==tis & md$site %in% sites
...
printf '%s\tEND\t%s\t%s\t%s\n' "$(date -Is)" "${SLURM_JOB_ID:-local}" "$GIT_HASH" "$(basename "$0")" >> runs.log
tail -40 results/position_effect/position_control.txt
